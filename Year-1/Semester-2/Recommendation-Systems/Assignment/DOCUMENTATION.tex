% !TeX program = pdflatex
% Converted from Markdown for Overleaf/pdfLaTeX.
% This file intentionally avoids fontspec and is compatible with pdfLaTeX.
\documentclass[12pt,a4paper]{article}

\usepackage[utf8]{inputenc}
\usepackage{cmap}
\usepackage[T2A]{fontenc}
\usepackage[english,bulgarian]{babel}
\selectlanguage{bulgarian}
\usepackage{geometry}
\geometry{margin=2.5cm}
\usepackage{microtype}
\usepackage{setspace}
\onehalfspacing
\setlength{\parindent}{0pt}
\setlength{\parskip}{6pt plus 2pt}
\usepackage{xcolor}
\usepackage{graphicx}
\usepackage{float}
\usepackage{caption}
\usepackage{amsmath}
\usepackage{longtable,booktabs,array,multirow}
\usepackage{etoolbox}
\AtBeginEnvironment{longtable}{\small}
\setlength{\LTleft}{0pt}
\setlength{\LTright}{0pt}
\usepackage{enumitem}
\setlist{itemsep=2pt,topsep=4pt}
\usepackage{fancyvrb}
\usepackage{fvextra}
\DefineVerbatimEnvironment{verbatim}{Verbatim}{breaklines=true,breakanywhere=true,fontsize=\small}

\providecommand{\tightlist}{\setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}}
\providecommand{\pandocbounded}[1]{#1}
\providecommand{\passthrough}[1]{#1}
\sloppy
\hbadness=10000
\hfuzz=5pt

\title{\textbf{Документация за курсов проект: NutriEvidence Agent}}
\author{
Явор Чамов\\
\small{Факултетен №: 4MI3400787}\\[0.5em]
\small{Преподавател: д-р Милен Чечев}
}
\date{\today}

\begin{document}
\maketitle

\begin{abstract}
Настоящият документ описва проекта \textit{NutriEvidence Agent} като образователна препоръчваща система за биомедицинска литература. Текстът е форматиран като статия и е подготвен за компилация с pdfLaTeX в Overleaf.
\end{abstract}

\tableofcontents
\newpage

\section{1. Резюме / Abstract}\label{ux440ux435ux437ux44eux43cux435-ux430ux431ux441ux442ux440ux430ux43aux442}

\textbf{NutriEvidence Agent} е образователна препоръчваща система за биомедицинска научна литература. Проектът има за цел да подпомага студенти, изследователи и потребители с научен интерес при откриване на релевантни публикации от PubMed по въпроси, свързани с хранене, обществено здраве, неврология и биомедицински теми.

Системата извлича статии от PubMed, съхранява ги локално, обработва заглавията и резюметата им, създава семантични представяния чрез sentence-transformer модел и препоръчва най-релевантните публикации чрез similarity-based подход. Като разширение системата изгражда Article--MeSH knowledge graph, обучава node2vec графови ембединги и използва графова близост за допълнително препоръчване на статии. Проектът включва и локален LLM слой чрез \textbf{Ollama + Llama 3.1 8B}, който се използва за анализ на потребителския въпрос, извличане на evidence елементи, обяснение на препоръките, reranking на кандидат-статии и safety проверка на финалния отговор.

Проектът включва и реализиран \textbf{OpenAI Judge} компонент за оценяване на релевантността на препоръчаните статии. Този компонент не участва в основното препоръчване и не се използва като медицински източник на истина. Неговата роля е да подпомага evaluation процеса чрез автоматично присвояване на relevance labels по скала 0--3, които по-късно могат да бъдат ръчно проверени и коригирани.

Проектът \textbf{не е медицинска диагностична система}. Той не поставя диагнози, не препоръчва лечение и не замества медицински специалист. Основната му цел е да бъде \textbf{research assistant} за намиране, подреждане, обобщаване и оценяване на релевантността на научна литература.

\vspace{0.5em}\hrule\vspace{1.0em}

\section{2. Проблем, който проектът решава}\label{ux43fux440ux43eux431ux43bux435ux43c-ux43aux43eux439ux442ux43e-ux43fux440ux43eux435ux43aux442ux44aux442-ux440ux435ux448ux430ux432ux430}

PubMed съдържа огромно количество биомедицински публикации. При търсене по дадена тема потребителят често получава голям брой резултати, които трудно се преглеждат ръчно. Това създава няколко проблема:

\begin{itemize}
\tightlist
\item
  трудно е бързо да се открият най-релевантните статии;
\item
  резултатите от търсачката невинаги са подредени според конкретния изследователски въпрос;
\item
  потребителят трябва сам да прецени кои публикации са review, systematic review, clinical trial или observational study;
\item
  липсва кратко обяснение защо дадена статия е релевантна;
\item
  при медицински теми е важно системата да не генерира неподкрепени твърдения или медицински съвети.
\end{itemize}

Проектът решава този проблем чрез комбиниране на:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{PubMed retrieval} --- извличане на реални публикации и метаданни;
\item
  \textbf{Semantic similarity recommender} --- препоръчване на статии чрез embedding близост между потребителския въпрос и title + abstract;
\item
  \textbf{Knowledge graph recommender} --- представяне на статии и MeSH термини като граф и използване на node2vec embeddings;
\item
  \textbf{Hybrid recommender} --- комбиниране на semantic и graph-based резултати;
\item
  \textbf{Local LLM agentic layer} --- използване на локален Llama модел за query planning, evidence extraction, reranking, explanations и safety checking;
\item
  \textbf{Evaluation module} --- изчисляване на Precision@K, nDCG@K и MRR върху ръчно или OpenAI Judge-assisted оценени препоръки.
\end{enumerate}

\vspace{0.5em}\hrule\vspace{1.0em}

\section{3. Основна цел на проекта}\label{ux43eux441ux43dux43eux432ux43dux430-ux446ux435ux43b-ux43dux430-ux43fux440ux43eux435ux43aux442ux430}

Основната цел е да се разработи \textbf{agentic RAG-based recommender system} за биомедицинска литература, която:

\begin{itemize}
\tightlist
\item
  приема потребителски научен въпрос;
\item
  извлича или използва кеширани PubMed статии;
\item
  подрежда публикации според релевантност;
\item
  препоръчва Top K статии;
\item
  обяснява защо са препоръчани;
\item
  обобщава evidence от намерените публикации;
\item
  използва локален LLM модел за подпомагане на анализа, без да генерира неподкрепени медицински твърдения.
\end{itemize}

\vspace{0.5em}\hrule\vspace{1.0em}

\section{4. Какво представлява RAG в проекта}\label{ux43aux430ux43aux432ux43e-ux43fux440ux435ux434ux441ux442ux430ux432ux43bux44fux432ux430-rag-ux432-ux43fux440ux43eux435ux43aux442ux430}

RAG означава \textbf{Retrieval-Augmented Generation}. В контекста на проекта това означава, че LLM моделът не отговаря само от собствените си предварително научени знания. Вместо това системата първо извлича релевантни PubMed публикации, подрежда ги чрез recommender алгоритми и чак след това подава най-релевантните статии като контекст към локалния LLM.

Общият процес е:

\begin{verbatim}
Потребителски въпрос
↓
Query Planner Agent
↓
PubMed retrieval / cached dataset
↓
Semantic recommender + graph recommender
↓
Top K релевантни статии
↓
LLM summary + explanations + safety checking
\end{verbatim}

По този начин отговорът е grounded върху реални статии и метаданни, а не върху свободна генерация от LLM.

\vspace{0.5em}\hrule\vspace{1.0em}

\section{5. Основни стъпки за решаване на проблема}\label{ux43eux441ux43dux43eux432ux43dux438-ux441ux442ux44aux43fux43aux438-ux437ux430-ux440ux435ux448ux430ux432ux430ux43dux435-ux43dux430-ux43fux440ux43eux431ux43bux435ux43cux430}

\subsection{5.1. Събиране на данни от PubMed}\label{ux441ux44aux431ux438ux440ux430ux43dux435-ux43dux430-ux434ux430ux43dux43dux438-ux43eux442-pubmed}

Първата стъпка е извличане на статии от PubMed чрез NCBI Entrez API. За всяка статия се съхраняват следните полета:

\begin{verbatim}
PMID
title
abstract
year
journal
authors
publication_types
mesh_terms
doi
source_query
\end{verbatim}

Статиите се кешират локално във файл:

\begin{verbatim}
data/pubmed_articles.json
\end{verbatim}

Това позволява приложението да работи и без постоянно извикване към PubMed API.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{5.2. Предварителна обработка на статиите}\label{ux43fux440ux435ux434ux432ux430ux440ux438ux442ux435ux43bux43dux430-ux43eux431ux440ux430ux431ux43eux442ux43aux430-ux43dux430-ux441ux442ux430ux442ux438ux438ux442ux435}

След извличане на статиите се извършва preprocessing:

\begin{itemize}
\tightlist
\item
  премахване на празни или невалидни записи;
\item
  нормализиране на whitespace;
\item
  обединяване на \texttt{title} и \texttt{abstract} в едно поле \texttt{document\_text};
\item
  гарантиране, че \texttt{mesh\_terms} винаги е списък;
\item
  подготовка на данните за embeddings и similarity ranking.
\end{itemize}

Примерно представяне:

\begin{verbatim}
document_text = title + ". " + abstract
\end{verbatim}

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{5.3. Semantic similarity recommender}\label{semantic-similarity-recommender}

Основният recommender в проекта е базиран на semantic similarity.

Използва се моделът:

\begin{verbatim}
sentence-transformers/all-MiniLM-L6-v2
\end{verbatim}

Той превръща всяка статия в embedding вектор на база \texttt{title\ +\ abstract}. Потребителският въпрос също се превръща в embedding. След това се изчислява cosine similarity между въпроса и всяка статия.

Общ процес:

\begin{verbatim}
User query
↓
Query embedding
↓
Article embeddings
↓
Cosine similarity
↓
Top K recommended articles
\end{verbatim}

Този подход е избран като основен, защото е лесен за имплементация, не изисква потребителски профили и е подходящ за content-based recommendation.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{5.4. MeSH overlap baseline}\label{mesh-overlap-baseline}

Като базов метод за сравнение се използва MeSH overlap recommender. Той измерва сходството между две статии чрез Jaccard similarity върху техните MeSH термини.

Формула:

\begin{equation*}
\mathrm{similarity}(A,B) = \frac{|\mathrm{MeSH}(A) \cap \mathrm{MeSH}(B)|}{|\mathrm{MeSH}(A) \cup \mathrm{MeSH}(B)|}
\end{equation*}

Този baseline е важен, защото позволява да се провери дали semantic embeddings дават по-добри препоръки от простото съвпадение на контролирани медицински термини.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{5.5. Knowledge graph и node2vec recommender}\label{knowledge-graph-ux438-node2vec-recommender}

Като разширение към проекта се изгражда Article--MeSH knowledge graph.

Възли:

\begin{verbatim}
Article
MeSHTerm
\end{verbatim}

Ребра:

\begin{verbatim}
Article --hasMeSHTerm--> MeSHTerm
\end{verbatim}

Пример:

\begin{verbatim}
article:12345678 --hasMeSHTerm--> mesh:cerebral_palsy
article:12345678 --hasMeSHTerm--> mesh:nutritional_status
article:12345678 --hasMeSHTerm--> mesh:child
\end{verbatim}

След това върху графа се обучава node2vec модел, който създава графови embeddings за възлите. Използват се само embeddings на article възлите, защото те са обектите, които системата препоръчва.

Графовият recommender намира най-близките статии по cosine similarity между node2vec embeddings.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{5.6. Hybrid recommender}\label{hybrid-recommender}

Hybrid recommender комбинира резултатите от semantic recommender и graph recommender.

Примерна формула:

\begin{verbatim}
final_score = 0.6 * semantic_score + 0.4 * graph_score
\end{verbatim}

Ако graph recommender не е наличен, системата трябва да може да работи само със semantic recommender.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{5.7. Local LLM layer чрез Ollama и Llama 3.1 8B}\label{local-llm-layer-ux447ux440ux435ux437-ollama-ux438-llama-3.1-8b}

Проектът използва локален LLM модел като задължителен MVP компонент:

\begin{verbatim}
Ollama
llama3.1:8b
\end{verbatim}

LLM моделът се използва за:

\begin{itemize}
\tightlist
\item
  анализ на потребителския въпрос;
\item
  извличане на PICO/PECO елементи;
\item
  генериране на PubMed query;
\item
  извличане на evidence от title и abstract;
\item
  обяснение на препоръките;
\item
  reranking на Top N кандидат-статии;
\item
  генериране на кратко evidence summary;
\item
  safety checking.
\end{itemize}

Важно ограничение: LLM не трябва да измисля статии, диагнози или медицински съвети. Той работи само върху вече извлечени статии и подадени metadata.

\vspace{0.5em}\hrule\vspace{1.0em}

\section{6. Дефиниране на хипотези}\label{ux434ux435ux444ux438ux43dux438ux440ux430ux43dux435-ux43dux430-ux445ux438ux43fux43eux442ux435ux437ux438}

В проекта могат да бъдат формулирани следните хипотези. На този етап те са проектни хипотези, а по-късно ще бъдат проверени експериментално чрез evaluation частта.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{Хипотеза H1: Semantic similarity е подходящ основен метод за препоръчване на PubMed статии}\label{ux445ux438ux43fux43eux442ux435ux437ux430-h1-semantic-similarity-ux435-ux43fux43eux434ux445ux43eux434ux44fux449-ux43eux441ux43dux43eux432ux435ux43d-ux43cux435ux442ux43eux434-ux437ux430-ux43fux440ux435ux43fux43eux440ux44aux447ux432ux430ux43dux435-ux43dux430-pubmed-ux441ux442ux430ux442ux438ux438}

\textbf{Твърдение:}\\
Embedding-based semantic similarity върху \texttt{title\ +\ abstract} ще връща релевантни статии за даден потребителски биомедицински въпрос.

\textbf{Аргументация:}\\
Заглавието и abstract-ът съдържат основната информация за статията. Sentence-transformer embeddings могат да улавят семантична близост дори когато думите не съвпадат напълно.

\textbf{Очакван резултат:}\\
Semantic recommender трябва да постига приемливи стойности на Precision@5 и nDCG@5.

\textbf{Критерий за отхвърляне:}\\
Хипотезата може да бъде отхвърлена, ако Top 5 резултатите често са слабо релевантни или нерелевантни според ръчна и/или OpenAI Judge-assisted annotation.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{Хипотеза H2: Semantic similarity ще бъде по-добър метод от MeSH overlap baseline}\label{ux445ux438ux43fux43eux442ux435ux437ux430-h2-semantic-similarity-ux449ux435-ux431ux44aux434ux435-ux43fux43e-ux434ux43eux431ux44aux440-ux43cux435ux442ux43eux434-ux43eux442-mesh-overlap-baseline}

\textbf{Твърдение:}\\
Semantic recommender ще връща по-добри резултати от прост baseline, базиран само на overlap между MeSH термини.

\textbf{Аргументация:}\\
MeSH термините са полезни, но често са твърде общи. Semantic embeddings използват повече информация, защото вземат предвид title и abstract.

\textbf{Очакван резултат:}\\
Semantic recommender трябва да има по-високи Precision@K и nDCG@K спрямо MeSH overlap baseline.

\textbf{Критерий за отхвърляне:}\\
Ако MeSH overlap baseline consistently постига равни или по-добри резултати от semantic recommender, хипотезата се отхвърля.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{Хипотеза H3: Knowledge graph + node2vec добавя полезна структурна информация}\label{ux445ux438ux43fux43eux442ux435ux437ux430-h3-knowledge-graph-node2vec-ux434ux43eux431ux430ux432ux44f-ux43fux43eux43bux435ux437ux43dux430-ux441ux442ux440ux443ux43aux442ux443ux440ux43dux430-ux438ux43dux444ux43eux440ux43cux430ux446ux438ux44f}

\textbf{Твърдение:}\\
Article--MeSH graph и node2vec embeddings могат да подобрят препоръките, защото улавят структурни връзки между статии и биомедицински понятия.

\textbf{Аргументация:}\\
Две статии може да са свързани чрез общи или близки MeSH термини, дори когато текстовете им не са много сходни.

\textbf{Очакван резултат:}\\
Graph recommender или hybrid recommender трябва да подобрява nDCG@5 или MRR спрямо само semantic similarity при част от query-тата.

\textbf{Критерий за отхвърляне:}\\
Ако graph-based резултатите са системно по-слаби и hybrid моделът не добавя подобрение, хипотезата се отхвърля за текущия dataset.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{Хипотеза H4: LLM reranking подобрява качеството на финалните Top K препоръки}\label{ux445ux438ux43fux43eux442ux435ux437ux430-h4-llm-reranking-ux43fux43eux434ux43eux431ux440ux44fux432ux430-ux43aux430ux447ux435ux441ux442ux432ux43eux442ux43e-ux43dux430-ux444ux438ux43dux430ux43bux43dux438ux442ux435-top-k-ux43fux440ux435ux43fux43eux440ux44aux43aux438}

\textbf{Твърдение:}\\
Локалният Llama модел може да подобри подреждането на Top 20 кандидат-статии, като избере най-подходящите Top 5 според population, exposure/intervention, outcome и тип на въпроса.

\textbf{Аргументация:}\\
Semantic similarity изчислява близост на векторно ниво, но LLM може да направи по-структурирана преценка върху metadata, abstract snippet и publication type.

\textbf{Очакван резултат:}\\
Semantic + LLM reranking или Hybrid + LLM reranking трябва да подобри nDCG@5 спрямо първоначалното алгоритмично подреждане.

\textbf{Критерий за отхвърляне:}\\
Ако LLM reranking понижава Precision@5 или nDCG@5, или често избира по-слабо релевантни статии, хипотезата се отхвърля.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{Хипотеза H5: Локален LLM е достатъчен за MVP agentic functionality}\label{ux445ux438ux43fux43eux442ux435ux437ux430-h5-ux43bux43eux43aux430ux43bux435ux43d-llm-ux435-ux434ux43eux441ux442ux430ux442ux44aux447ux435ux43d-ux437ux430-mvp-agentic-functionality}

\textbf{Твърдение:}\\
Локалният модел \texttt{llama3.1:8b} чрез Ollama е достатъчен за query planning, explanations, evidence extraction и safety checking в MVP версията.

\textbf{Аргументация:}\\
Проектът не изисква LLM да прави самостоятелна медицинска експертиза. Моделът трябва да изпълнява ограничени задачи върху подаден контекст.

\textbf{Очакван резултат:}\\
LLM трябва да връща достатъчно стабилни JSON outputs и разбираеми explanations.

\textbf{Критерий за отхвърляне:}\\
Ако локалният модел често връща невалиден JSON, hallucinated препоръки или неподходящи медицински твърдения, тази хипотеза се отхвърля и като бъдеща работа може да се добави OpenAI API provider.

\vspace{0.5em}\hrule\vspace{1.0em}

\section{7. Отхвърляне на хипотези}\label{ux43eux442ux445ux432ux44aux440ux43bux44fux43dux435-ux43dux430-ux445ux438ux43fux43eux442ux435ux437ux438}

На текущия етап проектът дефинира хипотезите, но не ги отхвърля окончателно, защото експериментите предстоят. Отхвърлянето ще се базира на evaluation резултати.

Предвижда се следната логика:

\begin{longtable}{@{}
  >{\raggedright\arraybackslash}p{0.28\linewidth}
  >{\raggedright\arraybackslash}p{0.32\linewidth}
  >{\raggedright\arraybackslash}p{0.32\linewidth}@{}}
\toprule
\textbf{Хипотеза} & \textbf{Как се проверява} & \textbf{Кога се отхвърля} \\
\midrule
\endhead
\bottomrule
\endlastfoot
\textbf{H1: Semantic similarity е подходящ метод} & Precision@5, nDCG@5 върху semantic results & Ако повечето Top 5 резултати са нерелевантни \\
\textbf{H2: Semantic > MeSH baseline} & Сравнение Semantic vs MeSH overlap & Ако MeSH baseline е равен или по-добър \\
\textbf{H3: Graph/node2vec добавя стойност} & Graph и Hybrid спрямо Semantic & Ако няма подобрение или има влошаване \\
\textbf{H4: LLM reranking подобрява Top K} & Semantic/Hybrid vs LLM reranked & Ако LLM reranking намали метриките \\
\textbf{H5: Local Llama е достатъчен} & JSON validity, explanation quality, safety checks & Ако Llama outputs са нестабилни или небезопасни \\
\end{longtable}

След добавяне на експериментите тази секция ще бъде разширена с реални резултати, таблици и заключения.

\vspace{0.5em}\hrule\vspace{1.0em}

\section{8. Реализация}\label{ux440ux435ux430ux43bux438ux437ux430ux446ux438ux44f}

\subsection{8.1. Използвани технологии}\label{ux438ux437ux43fux43eux43bux437ux432ux430ux43dux438-ux442ux435ux445ux43dux43eux43bux43eux433ux438ux438}

Проектът използва следния технологичен стек:

\begin{verbatim}
Python 3.10+
Streamlit
pandas
numpy
scikit-learn
sentence-transformers
Biopython
NetworkX
node2vec
gensim
python-dotenv
requests
Ollama
llama3.1:8b
OpenAI API за evaluation judge
\end{verbatim}

OpenAI API не е част от основния MVP runtime за генериране на отговори и препоръки. Основният LLM runtime остава локалният Llama модел чрез Ollama. OpenAI API обаче вече е реализиран и се използва като \textbf{evaluation judge} за подпомагане на relevance annotation на препоръките.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.2. Структура на проекта}\label{ux441ux442ux440ux443ux43aux442ux443ux440ux430-ux43dux430-ux43fux440ux43eux435ux43aux442ux430}

Планираната структура е:

\begin{verbatim}
nutri-evidence-agent/
|
+-- app.py
+-- requirements.txt
+-- .env.example
+-- README.md
|
+-- src/
|   +-- retrieval/
|   +-- preprocessing/
|   +-- recommenders/
|   +-- graph/
|   +-- llm/
|   +-- agents/
|   +-- evaluation/
|   `-- utils/
|
+-- data/
|   +-- sample_queries.json
|   +-- pubmed_articles.json
|   +-- evaluation_annotations.csv
|   `-- artifacts/
|
+-- scripts/
|
+-- notebooks/
|
`-- docs/
\end{verbatim}

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.3. PubMed retrieval модул}\label{pubmed-retrieval-ux43cux43eux434ux443ux43b}

Модулът за PubMed retrieval използва Biopython Entrez API.

Основни функции:

\begin{itemize}
\tightlist
\item
  \texttt{search(query,\ max\_results)} --- връща списък от PMID;
\item
  \texttt{fetch\_details(pmids)} --- извлича metadata за статии;
\item
  \texttt{search\_and\_fetch(query,\ max\_results)} --- комбинира търсене и извличане.
\end{itemize}

Резултатите се нормализират до общ article schema и се кешират локално.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.4. Cache модул}\label{cache-ux43cux43eux434ux443ux43b}

Cache модулът отговаря за:

\begin{itemize}
\tightlist
\item
  зареждане на локални статии;
\item
  записване в JSON;
\item
  merge на нови и съществуващи статии;
\item
  deduplication по PMID.
\end{itemize}

Това позволява многократно стартиране на проекта без дублиране на записи.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.5. Preprocessing модул}\label{preprocessing-ux43cux43eux434ux443ux43b}

Preprocessing модулът:

\begin{itemize}
\tightlist
\item
  нормализира текст;
\item
  създава \texttt{document\_text};
\item
  филтрира невалидни записи;
\item
  подготвя данните за embeddings и ranking.
\end{itemize}

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.6. Semantic recommender модул}\label{semantic-recommender-ux43cux43eux434ux443ux43b}

Semantic recommender:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  зарежда sentence-transformer модел;
\item
  създава embeddings за всички статии;
\item
  създава embedding за потребителския въпрос;
\item
  изчислява cosine similarity;
\item
  връща Top K препоръки.
\end{enumerate}

Резултатите съдържат:

\begin{verbatim}
pmid
title
abstract
year
journal
mesh_terms
score
method
\end{verbatim}

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.7. MeSH overlap baseline}\label{mesh-overlap-baseline-1}

MeSH overlap recommender служи като baseline. Той използва Jaccard similarity между множествата от MeSH термини на статии.

Този baseline ще бъде използван за сравнение със semantic recommender.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.8. Knowledge graph модул}\label{knowledge-graph-ux43cux43eux434ux443ux43b}

Knowledge graph модулът изгражда Article--MeSH граф чрез NetworkX.

Основни компоненти:

\begin{itemize}
\tightlist
\item
  \texttt{ArticleMeshGraphBuilder};
\item
  \texttt{Node2VecTrainer};
\item
  \texttt{GraphRecommender}.
\end{itemize}

Графът се записва като artifact, а node2vec embeddings се запазват за повторна употреба.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.9. Hybrid recommender}\label{hybrid-recommender-1}

Hybrid recommender комбинира semantic и graph score. Основната идея е да се използва както текстова семантична близост, така и структурна близост в knowledge graph.

Ако graph model липсва, системата fallback-ва към semantic recommender.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.10. Ollama client}\label{ollama-client}

LLM слоят използва локален Ollama HTTP API.

Конфигурация:

\begin{verbatim}
USE_LLM=true
OLLAMA_BASE_URL=http://localhost:11434
OLLAMA_MODEL=llama3.1:8b
\end{verbatim}

Ollama client трябва да поддържа:

\begin{itemize}
\tightlist
\item
  text generation;
\item
  JSON generation;
\item
  обработка на connection errors;
\item
  fallback при липсващ Ollama server.
\end{itemize}

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.11. Query Planner Agent}\label{query-planner-agent}

Query Planner Agent анализира потребителския въпрос и извлича:

\begin{verbatim}
population
exposure
intervention
outcome
question_type
pubmed_query
\end{verbatim}

Пример:

\begin{verbatim}
Does vitamin D supplementation improve bone health in children with cerebral palsy?
\end{verbatim}

Изход:

\begin{verbatim}
{
  "population": "children with cerebral palsy",
  "exposure": null,
  "intervention": "vitamin D supplementation",
  "outcome": "bone health",
  "question_type": "effectiveness",
  "pubmed_query": "vitamin D supplementation bone health children cerebral palsy"
}
\end{verbatim}

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.12. Evidence Extraction Agent}\label{evidence-extraction-agent}

Evidence Extraction Agent извлича структурирана информация от title и abstract на статия:

\begin{verbatim}
population
exposure_or_intervention
outcome
main_finding
limitations
\end{verbatim}

Той не трябва да измисля резултати, които не присъстват в abstract-а.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.13. Recommendation Explanation Agent}\label{recommendation-explanation-agent}

Този агент генерира кратко обяснение защо дадена статия е препоръчана.

Пример:

\begin{verbatim}
This article is recommended because it matches the topic of nutritional status in children with cerebral palsy and shares MeSH terms related to Cerebral Palsy, Child, and Nutritional Status.
\end{verbatim}

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.14. LLM Recommendation Reranker}\label{llm-recommendation-reranker}

LLM reranker получава Top N кандидат-статии от semantic, graph или hybrid recommender и избира финални Top K препоръки.

Важно: той не вижда целия dataset и няма право да измисля нови PMID или заглавия.

Процес:

\begin{verbatim}
Algorithmic Top 20 candidates
↓
Compact metadata to Llama
↓
LLM reranking
↓
Validated Top 5 recommendations
\end{verbatim}

Ако LLM върне невалиден JSON или измислен PMID, системата използва fallback към оригиналното алгоритмично подреждане.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.15. Answer Generation Agent}\label{answer-generation-agent}

Answer Generator създава структуриран отговор:

\begin{verbatim}
## Short Answer
## Evidence Summary
## Recommended Papers
## Limitations
## Safety Note
\end{verbatim}

Той трябва да обобщава само предоставените статии и evidence items.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.16. Safety Checker Agent}\label{safety-checker-agent}

Safety Checker гарантира, че финалният output:

\begin{itemize}
\tightlist
\item
  не съдържа диагностика;
\item
  не съдържа персонализирано лечение;
\item
  не казва на потребителя какво лекарство да приема;
\item
  съдържа safety disclaimer.
\end{itemize}

Задължителен disclaimer:

\begin{verbatim}
This output is for educational and research purposes only and should not be interpreted as medical advice, diagnosis, or treatment recommendation.
\end{verbatim}

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.17. Streamlit интерфейс}\label{streamlit-ux438ux43dux442ux435ux440ux444ux435ux439ux441}

Streamlit UI трябва да поддържа два режима:

\subsubsection{Mode A: Search by research question}\label{mode-a-search-by-research-question}

Потребителят въвежда въпрос, например:

\begin{verbatim}
What is the evidence linking gut microbiome and Parkinson's disease?
\end{verbatim}

Системата:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  анализира въпроса;
\item
  генерира PubMed query;
\item
  търси в cached dataset или PubMed;
\item
  препоръчва Top 5 статии;
\item
  генерира explanation и summary;
\item
  показва safety note.
\end{enumerate}

\subsubsection{Mode B: Recommend by selected article}\label{mode-b-recommend-by-selected-article}

Потребителят избира статия от dropdown. Системата препоръчва сходни статии чрез semantic, graph и hybrid recommender.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{8.18. OpenAI Judge модул}\label{openai-judge-ux43cux43eux434ux443ux43b}

OpenAI Judge модулът е реализиран като част от evaluation слоя на проекта. Той се използва за автоматизирано подпомагане на relevance annotation, когато се оценява качеството на препоръките.

Основната му задача е да получи:

\begin{verbatim}
потребителски query
PMID
заглавие на статия
abstract snippet
MeSH terms
publication types
semantic score
rank
\end{verbatim}

и да върне:

\begin{verbatim}
judge_relevance: стойност от 0 до 3
judge_reason: кратко обяснение на оценката
judge_model: използвания OpenAI модел
\end{verbatim}

Този модул се използва само за evaluation. Той не участва в избора на препоръки в потребителския интерфейс и не генерира медицински отговори. За safety и методологична коректност OpenAI Judge оценките могат да бъдат прегледани и коригирани ръчно чрез \texttt{human\_relevance} и \texttt{human\_notes}.

\vspace{0.5em}\hrule\vspace{1.0em}

\section{9. Оценяване на системата}\label{ux43eux446ux435ux43dux44fux432ux430ux43dux435-ux43dux430-ux441ux438ux441ux442ux435ux43cux430ux442ux430}

В проекта е реализирана експериментална evaluation част. Тя включва както методологията за оценяване, така и реални резултати от OpenAI-assisted relevance annotation върху два типа сценарии:

\begin{itemize}
\tightlist
\item
  \textbf{Mode A: Search by research question} --- сравнение между първоначален semantic ranking и Semantic + LLM reranking за няколко research questions;
\item
  \textbf{Mode B: Recommend by selected article} --- сравнение между MeSH overlap baseline, semantic similarity, graph/node2vec, hybrid recommender и техните LLM-reranked варианти за избрана seed статия.
\end{itemize}

Оценяването използва ranking метрики като Precision@5, Precision@10, nDCG@5, nDCG@10 и MRR. Релевантността е зададена по скала 0-3 чрез OpenAI Judge, като системата поддържа и ръчна корекция чрез \texttt{human\_relevance}.

\subsection{9.1. Основен оценяван метод}\label{ux43eux441ux43dux43eux432ux435ux43d-ux43eux446ux435ux43dux44fux432ux430ux43d-ux43cux435ux442ux43eux434}

Основният метод за оценка в query-based експериментите е:

\begin{verbatim}
Semantic Similarity Recommender
\end{verbatim}

Той се оценява чрез Top 10 препоръки за набор от тестови въпроси. Допълнително се оценява и LLM reranking слой, който получава само вече намерените algorithmic candidates и ги пренарежда до финални Top 5 препоръки.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{9.2. Релевантност}\label{ux440ux435ux43bux435ux432ux430ux43dux442ux43dux43eux441ux442}

Всяка препоръчана статия ще бъде оценена по скала:

\begin{verbatim}
0 = not relevant
1 = somewhat relevant
2 = relevant
3 = highly relevant
\end{verbatim}

За binary metrics ще се използва:

\begin{verbatim}
relevance >= 2 = relevant
\end{verbatim}

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{9.3. Метрики}\label{ux43cux435ux442ux440ux438ux43aux438}

Планираните метрики са:

\begin{verbatim}
Precision@5
Precision@10
nDCG@5
nDCG@10
MRR
\end{verbatim}

При binary метриките се приема:

\begin{verbatim}
relevance >= 2 = relevant
\end{verbatim}

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{9.4. OpenAI Judge за evaluation}\label{openai-judge-ux437ux430-evaluation}

В проекта е реализиран \textbf{OpenAI Judge} компонент, който се използва за подпомагане на оценяването на препоръчващата система. Той не е част от основния recommender runtime и не избира препоръките, които потребителят получава. Основният recommender остава semantic similarity подходът, допълнен от graph/hybrid/LLM reranking модули.

OpenAI Judge се използва след генериране на кандидат-препоръките. За всяка двойка \texttt{query\ +\ recommended\ article} той присвоява relevance score по скала 0--3 и кратко обяснение на оценката.

Процесът е:

\begin{verbatim}
Evaluation queries
↓
Semantic recommender връща Top 10 статии за всяко query
↓
OpenAI Judge оценява всяка препоръчана статия по скала 0–3
↓
Оценките се записват в evaluation annotations файл
↓
Изчисляват се Precision@5, Precision@10, nDCG@5, nDCG@10 и MRR
\end{verbatim}

OpenAI Judge връща примерно:

\begin{verbatim}
{
  "relevance": 3,
  "reason": "The article directly matches the query because it focuses on children with cerebral palsy and nutritional status.",
  "judge_model": "gpt-4o-mini"
}
\end{verbatim}

Резултатите от judge компонента се записват в CSV файл със следните полета:

\begin{verbatim}
query_id
query
method
rank
pmid
title
year
journal
semantic_score
judge_relevance
judge_reason
judge_model
human_relevance
human_notes
\end{verbatim}

Полето \texttt{human\_relevance} позволява ръчна проверка и корекция. Ако има ръчна оценка, тя има приоритет пред \texttt{judge\_relevance} при изчисляване на метриките. Ако няма ръчна оценка, се използва OpenAI Judge label.

Важно: OpenAI Judge \textbf{не е източник на медицинска истина}. Той е помощен инструмент за relevance annotation и оценяване на recommender ranking-а. Неговите оценки трябва да се разглеждат като LLM-assisted labels, а не като абсолютен ground truth.

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{9.5. Реални evaluation резултати}\label{ux440ux435ux430ux43bux43dux438-evaluation-ux440ux435ux437ux443ux43bux442ux430ux442ux438}

За експерименталната оценка са използвани пет OpenAI-annotated файла:

\begin{verbatim}
evaluation_annotations_openai_q2.csv
evaluation_annotations_openai_q3.csv
evaluation_annotations_openai_q4.csv
evaluation_annotations_openai_q5.csv
evaluation_annotations_openai_q6.csv
\end{verbatim}

Общо са анализирани \textbf{75 judged rows}:

\begin{itemize}
\tightlist
\item
  50 резултата от първоначалния \textbf{Semantic Similarity} ranking;
\item
  25 резултата от \textbf{Semantic + LLM rerank};
\item
  5 evaluation queries;
\item
  без ръчни override оценки в \texttt{human\_relevance}, т.е. използвани са директно \texttt{judge\_relevance} label-ите.
\end{itemize}

При binary метриките е използван праг:

\begin{verbatim}
relevance >= 2 = relevant
\end{verbatim}

Важно уточнение: LLM reranker връща финални Top 5 препоръки. Затова при \texttt{semantic+llm\_rerank} метриките \texttt{Precision@10} и \texttt{nDCG@10} се изчисляват върху наличните 5 резултата, защото няма 10 финални reranked статии.

\subsubsection{9.5.1. Средни резултати}\label{ux441ux440ux435ux434ux43dux438-ux440ux435ux437ux443ux43bux442ux430ux442ux438}

\begin{longtable}[]{@{}lrrrrr@{}}
\toprule\noalign{}
Метод & Precision@5 & Precision@10 & nDCG@5 & nDCG@10 & MRR \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Semantic Similarity & 0.640 & 0.560 & 0.935 & 0.916 & 0.900 \\
Semantic + LLM rerank & 0.720 & 0.720 & 0.990 & 0.990 & 1.000 \\
\end{longtable}

Графика на средните метрики:

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\linewidth]{openai_evaluation_average_metrics.png}
\caption{Average evaluation metrics}
\end{figure}

Резултатите показват, че LLM reranker подобрява всички отчетени метрики в този evaluation subset. Най-видимото подобрение е при:

\begin{itemize}
\tightlist
\item
  \texttt{Precision@5}: 0.640 \textrightarrow{} 0.720;
\item
  \texttt{Precision@10}: 0.560 \textrightarrow{} 0.720;
\item
  \texttt{nDCG@5}: 0.935 \textrightarrow{} 0.990;
\item
  \texttt{MRR}: 0.900 \textrightarrow{} 1.000.
\end{itemize}

Това означава, че reranker-ът не създава нови статии, а по-добре подрежда вече намерените semantic кандидати така, че по-релевантните статии да се появяват по-рано.

\subsubsection{9.5.2. Резултати по query}\label{ux440ux435ux437ux443ux43bux442ux430ux442ux438-ux43fux43e-query}

\begin{longtable}[]{@{}llrrrrr@{}}
\toprule\noalign{}
Query & Метод & Precision@5 & Precision@10 & nDCG@5 & nDCG@10 & MRR \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
q2 & Semantic Similarity & 1.000 & 1.000 & 1.000 & 1.000 & 1.000 \\
q2 & Semantic + LLM rerank & 1.000 & 1.000 & 1.000 & 1.000 & 1.000 \\
q3 & Semantic Similarity & 0.800 & 0.600 & 0.922 & 0.919 & 1.000 \\
q3 & Semantic + LLM rerank & 1.000 & 1.000 & 0.984 & 0.984 & 1.000 \\
q4 & Semantic Similarity & 0.600 & 0.600 & 0.759 & 0.735 & 0.500 \\
q4 & Semantic + LLM rerank & 0.600 & 0.600 & 0.966 & 0.966 & 1.000 \\
q5 & Semantic Similarity & 0.400 & 0.400 & 1.000 & 0.946 & 1.000 \\
q5 & Semantic + LLM rerank & 0.400 & 0.400 & 1.000 & 1.000 & 1.000 \\
q6 & Semantic Similarity & 0.400 & 0.200 & 0.993 & 0.981 & 1.000 \\
q6 & Semantic + LLM rerank & 0.600 & 0.600 & 1.000 & 1.000 & 1.000 \\
\end{longtable}

Графика за \texttt{nDCG@5} по query:


\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\linewidth]{openai_evaluation_per_query_ndcg5.png}
\caption{nDCG@5 по query}
\end{figure}

Наблюдения:

\begin{itemize}
\tightlist
\item
  q2 е лесен пример за semantic recommender-а: всички Top 10 резултати са оценени като релевантни или високо релевантни, затова LLM reranker няма голямо пространство за подобрение.
\item
  q3 показва ясно подобрение: LLM reranker избира по-фокусирани статии за feeding difficulties при cerebral palsy и повишава Precision@5 до 1.000.
\item
  q4 има по-ниска начална подредба, но reranker-ът поставя силно релевантните pregnancy/neurodevelopment статии по-напред; MRR се подобрява от 0.500 до 1.000.
\item
  q5 е по-труден query: темата urbanization + obesity-related dietary behaviors е широка, затова и двата метода намират частично релевантни статии, но precision остава 0.400.
\item
  q6 е по-специфичен към ultra-processed foods и urban living; reranker-ът подобрява Precision@5 от 0.400 до 0.600.
\end{itemize}

\subsubsection{9.5.3. Поведение на algorithmic ranking-а}\label{ux43fux43eux432ux435ux434ux435ux43dux438ux435-ux43dux430-algorithmic-ranking-ux430}

Първоначалният semantic recommender работи чрез cosine similarity върху embeddings на заглавие и abstract. Той се държи стабилно при query-та, които имат директно лексикално и тематично съвпадение с наличните статии. Това се вижда при q2, където \texttt{gut\ microbiome} и \texttt{Parkinson\ disease} са силно представени в titles, abstracts и MeSH terms.

При по-широки или по-комплексни query-та semantic ranking-ът често намира тематично близки статии, но не винаги подрежда най-подходящите на първите позиции. Например при q4 първият semantic резултат е оценен с relevance 1, защото е по-общо свързан с anti-inflammatory diet, но не отговаря директно на pregnancy + child neurodevelopment.

\subsubsection{9.5.4. Поведение на LLM reranker-а}\label{ux43fux43eux432ux435ux434ux435ux43dux438ux435-ux43dux430-llm-reranker-ux430}

LLM reranker-ът получава само Top 10 algorithmic кандидата и избира финални Top 5. Той не може да измисля нови PMID-и и не може да излиза извън кандидатния списък.

Типични reranking ефекти:

\begin{itemize}
\tightlist
\item
  при q2 статия с semantic rank 10 е преместена до LLM rank 3, защото е пряко за Parkinson disease и gut microbiome;
\item
  при q3 статии със semantic rank 7 и 8 са преместени в Top 5, защото са по-практически релевантни за nutritional/feeding management;
\item
  при q4 статия със semantic rank 10 е преместена до LLM rank 2, защото е директно за Mediterranean-style diet during pregnancy и neurodevelopmental disabilities;
\item
  при q6 reranker-ът запазва първите urbanization/dietary-pattern статии, но добавя и по-ниско ранкирани кандидати в Top 5.
\end{itemize}

Това показва, че LLM reranker-ът действа като втори ranking слой: semantic recommender-ът осигурява recall, а LLM reranker-ът подобрява precision и ordering в рамките на вече намерените кандидати.

\subsubsection{9.5.5. Поведение на OpenAI Judge}\label{ux43fux43eux432ux435ux434ux435ux43dux438ux435-ux43dux430-openai-judge}

OpenAI Judge използва скала 0-3 и оценява всяка двойка \texttt{query\ +\ recommended\ article}. Разпределението на labels е:

\begin{longtable}[]{@{}lrrrr@{}}
\toprule\noalign{}
Метод & Label 0 & Label 1 & Label 2 & Label 3 \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Semantic Similarity & 6 & 16 & 12 & 16 \\
Semantic + LLM rerank & 1 & 6 & 7 & 11 \\
\end{longtable}

Графика на разпределението:

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\linewidth]{openai_evaluation_relevance_distribution.png}
\caption{Разпределение на relevance labels}
\end{figure}

След LLM reranking делът на label 3 резултатите се увеличава от 32\% до 44\%, а label 0 резултатите намаляват от 12\% до 4\%. Това подкрепя извода, че reranker-ът филтрира част от по-слабите semantic кандидати.

Въпреки това OpenAI Judge трябва да се разглежда като \textbf{assisted annotation}, а не като окончателна истина. За по-надеждна evaluation част от labels трябва да бъдат прегледани ръчно. В проекта е предвидено поле \texttt{human\_relevance}, което има приоритет пред \texttt{judge\_relevance}.

\subsubsection{9.5.6. Генерирани evaluation artifacts}\label{ux433ux435ux43dux435ux440ux438ux440ux430ux43dux438-evaluation-artifacts}

Анализът генерира следните файлове:

\begin{verbatim}
nutri-evidence-agent/docs/evaluation/openai_evaluation_average_metrics.csv
nutri-evidence-agent/docs/evaluation/openai_evaluation_per_query_metrics.csv
nutri-evidence-agent/docs/evaluation/openai_evaluation_rank_movements.csv
nutri-evidence-agent/docs/evaluation/openai_evaluation_relevance_distribution.csv
nutri-evidence-agent/docs/evaluation/openai_evaluation_query_summary.csv
nutri-evidence-agent/docs/evaluation/openai_evaluation_average_metrics.svg
nutri-evidence-agent/docs/evaluation/openai_evaluation_per_query_ndcg5.svg
nutri-evidence-agent/docs/evaluation/openai_evaluation_relevance_distribution.svg
\end{verbatim}

Командата за възпроизвеждане на анализа е:

\begin{verbatim}
cd Year-1/Semester-2/Recommendation-Systems/Assignment/nutri-evidence-agent
python3 scripts/analyze_openai_evaluation_results.py
\end{verbatim}

\vspace{0.5em}\hrule\vspace{1.0em}

\subsection{9.6. Evaluation за Mode B: Recommend by selected article}\label{evaluation-ux437ux430-mode-b-recommend-by-selected-article}

Допълнително е направен отделен evaluation експеримент за \textbf{Mode B: Recommend by selected article}. Тук потребителят избира seed article, а системата препоръчва подобни статии.

Използваният seed article е:

\begin{verbatim}
PMID 37656239
Low skeletal muscle mass and liver fibrosis in children with cerebral palsy.
\end{verbatim}

Evaluation query-то, подадено към OpenAI Judge, е формулирано като article-similarity задача:

\begin{verbatim}
Find articles similar to PMID 37656239: Low skeletal muscle mass and liver fibrosis in children with cerebral palsy.
\end{verbatim}

Използваните annotation файлове са:

\begin{verbatim}
evaluation_annotations_openai_mesh_overlap_article_37656239.csv
evaluation_annotations_openai_semantic_article_37656239.csv
evaluation_annotations_openai_graph_article_37656239.csv
evaluation_annotations_openai_hybrid_article_37656239.csv
\end{verbatim}

Общо са анализирани \textbf{50 judged rows}:

\begin{itemize}
\tightlist
\item
  5 MeSH overlap baseline резултата;
\item
  10 semantic резултата + 5 semantic LLM-reranked резултата;
\item
  10 graph/node2vec резултата + 5 graph LLM-reranked резултата;
\item
  10 hybrid резултата + 5 hybrid LLM-reranked резултата.
\end{itemize}

Няма ръчни override оценки в \texttt{human\_relevance}, затова метриките използват директно \texttt{judge\_relevance}.

\subsubsection{9.6.1. Метрики за Mode B}\label{ux43cux435ux442ux440ux438ux43aux438-ux437ux430-mode-b}

\begin{longtable}[]{@{}lrrrrr@{}}
\toprule\noalign{}
Метод & Precision@5 & Precision@10 & nDCG@5 & nDCG@10 & MRR \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
MeSH overlap baseline & 0.200 & 0.200 & 0.678 & 0.678 & 0.200 \\
Semantic Similarity & 0.200 & 0.300 & 0.798 & 0.774 & 0.333 \\
Semantic + LLM rerank & 0.600 & 0.600 & 1.000 & 1.000 & 1.000 \\
Graph node2vec & 0.200 & 0.200 & 0.821 & 0.775 & 0.500 \\
Graph + LLM rerank & 0.200 & 0.200 & 0.706 & 0.706 & 0.200 \\
Hybrid & 0.200 & 0.200 & 0.770 & 0.763 & 0.250 \\
Hybrid + LLM rerank & 0.400 & 0.400 & 0.839 & 0.839 & 0.500 \\
\end{longtable}

Графика на метриките:

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\linewidth]{article_37656239/openai_evaluation_average_metrics.png}
\caption{Mode B average metrics}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\linewidth]{article_37656239/openai_evaluation_per_query_ndcg5.png}
\caption{nDCG@5 article-based}
\end{figure}

Важно уточнение: MeSH overlap и LLM-reranked вариантите имат само Top 5 резултата в този експеримент, затова \texttt{Precision@10} и \texttt{nDCG@10} се изчисляват върху наличните редове.

\subsubsection{9.6.2. Поведение на отделните алгоритми}\label{ux43fux43eux432ux435ux434ux435ux43dux438ux435-ux43dux430-ux43eux442ux434ux435ux43bux43dux438ux442ux435-ux430ux43bux433ux43eux440ux438ux442ux43cux438}

\textbf{MeSH overlap baseline} работи като проста концептуална baseline система. Той намира статии с припокриващи се MeSH термини, например \texttt{Cerebral\ Palsy}, \texttt{Child}, \texttt{Body\ Composition} или близки клинични понятия. В този експеримент baseline-ът намира само 1 релевантна статия от 5. Това показва ограничението на чистия MeSH overlap: той улавя общата област, но трудно различава дали статията е за skeletal muscle mass и liver fibrosis.

\textbf{Semantic Similarity} показва по-добър recall от MeSH baseline. Top 10 semantic резултатите съдържат 3 релевантни статии, но само 1 от първите 5 е релевантна според прага \texttt{relevance\ \textgreater{}=\ 2}. Това означава, че semantic embedding-ите намират тематично близки статии за body composition и cerebral palsy, но не винаги ги поставят най-горе.

\textbf{Semantic + LLM rerank} е най-силният вариант в този конкретен Mode B експеримент. Precision@5 се повишава от 0.200 до 0.600, а MRR достига 1.000. LLM reranker-ът избира статии, които са по-близки до seed article-а по теми като body composition, muscle tone и nutritional/body composition assessment.

\textbf{Graph node2vec} постига по-добър \texttt{nDCG@5} от MeSH baseline и semantic-only, но precision остава нисък. Graph моделът намира статии, които са структурно близки в Article-MeSH graph, но някои от тях са за bone mineral density или general growth/nutrition disorders, което е близко до cerebral palsy, но не директно до skeletal muscle mass + liver fibrosis.

\textbf{Graph + LLM rerank} в този експеримент не подобрява graph резултатите. Precision остава 0.200, а nDCG@5 пада от 0.821 до 0.706. Това показва важен риск: ако graph кандидатният списък съдържа много частично релевантни, но не директни статии, LLM reranker-ът не може да компенсира напълно слаб candidate pool.

\textbf{Hybrid} комбинира semantic и graph сигнали, но началният hybrid ranking тук остава близък до graph/semantic по качество. \textbf{Hybrid + LLM rerank} подобрява Precision@5 до 0.400 и MRR до 0.500, но не достига качеството на Semantic + LLM rerank. Това подсказва, че за този seed article semantic кандидатите са по-подходяща основа за reranking от graph/hybrid кандидатите.

\subsubsection{9.6.3. Поведение на LLM reranker-а}\label{ux43fux43eux432ux435ux434ux435ux43dux438ux435-ux43dux430-llm-reranker-ux430-1}

LLM reranker-ът е най-полезен, когато candidate pool вече съдържа достатъчно директно релевантни статии. При semantic кандидатите той премества нагоре:

\begin{itemize}
\tightlist
\item
  статии за body composition и muscle tone при children with cerebral palsy;
\item
  статии за functional-level based nutritional/body composition assessment;
\item
  статии за assessment methods за body composition при cerebral palsy.
\end{itemize}

При graph кандидатите reranker-ът често работи с по-шумен candidate pool, съдържащ bone mineral density и assisted standing теми. Те са свързани с cerebral palsy, но OpenAI Judge ги оценява като само частично релевантни, защото не покриват skeletal muscle mass и liver fibrosis.

\subsubsection{9.6.4. Поведение на OpenAI Judge}\label{ux43fux43eux432ux435ux434ux435ux43dux438ux435-ux43dux430-openai-judge-1}

Разпределението на OpenAI Judge labels за Mode B е:

\begin{longtable}[]{@{}lrrrr@{}}
\toprule\noalign{}
Метод & Label 0 & Label 1 & Label 2 & Label 3 \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
MeSH overlap baseline & 1 & 3 & 1 & 0 \\
Semantic Similarity & 0 & 7 & 3 & 0 \\
Semantic + LLM rerank & 0 & 2 & 3 & 0 \\
Graph node2vec & 2 & 6 & 2 & 0 \\
Graph + LLM rerank & 1 & 3 & 1 & 0 \\
Hybrid & 1 & 7 & 2 & 0 \\
Hybrid + LLM rerank & 0 & 3 & 2 & 0 \\
\end{longtable}

Графика на разпределението:

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\linewidth]{article_37656239/openai_evaluation_relevance_distribution.png}
\caption{Mode B relevance distribution}
\end{figure}

Прави впечатление, че няма label 3 резултати. Това е очаквано за този seed article, защото query-то е доста специфично: \texttt{low\ skeletal\ muscle\ mass} + \texttt{liver\ fibrosis} + \texttt{children\ with\ cerebral\ palsy}. Повечето кандидати покриват само част от темата, най-често \texttt{cerebral\ palsy} и \texttt{body\ composition}, но не и \texttt{liver\ fibrosis}.

Това поведение е полезно за анализа: OpenAI Judge не оценява високо само защото статията е за cerebral palsy. Той наказва липсата на ключови компоненти от seed article-а.

\subsubsection{9.6.5. Извод от Mode B експеримента}\label{ux438ux437ux432ux43eux434-ux43eux442-mode-b-ux435ux43aux441ux43fux435ux440ux438ux43cux435ux43dux442ux430}

За seed article \texttt{37656239} най-добре се представя:

\begin{verbatim}
Semantic Similarity + LLM rerank
\end{verbatim}

Този резултат подкрепя архитектурното решение първо да се използва semantic recommender за добър candidate recall, а след това LLM reranker да подреди Top 10 кандидатите според по-фина релевантност към seed article-а.

Graph и hybrid подходите остават полезни като алтернативни retrieval сигнали, но този експеримент показва, че graph-based близостта по MeSH terms не винаги е достатъчна, когато seed article-ът има много специфичен outcome.

\subsubsection{9.6.6. Генерирани Mode B evaluation artifacts}\label{ux433ux435ux43dux435ux440ux438ux440ux430ux43dux438-mode-b-evaluation-artifacts}

Анализът генерира следните файлове:

\begin{verbatim}
nutri-evidence-agent/docs/evaluation/article_37656239/openai_evaluation_average_metrics.csv
nutri-evidence-agent/docs/evaluation/article_37656239/openai_evaluation_per_query_metrics.csv
nutri-evidence-agent/docs/evaluation/article_37656239/openai_evaluation_rank_movements.csv
nutri-evidence-agent/docs/evaluation/article_37656239/openai_evaluation_relevance_distribution.csv
nutri-evidence-agent/docs/evaluation/article_37656239/openai_evaluation_query_summary.csv
nutri-evidence-agent/docs/evaluation/article_37656239/openai_evaluation_average_metrics.svg
nutri-evidence-agent/docs/evaluation/article_37656239/openai_evaluation_per_query_ndcg5.svg
nutri-evidence-agent/docs/evaluation/article_37656239/openai_evaluation_relevance_distribution.svg
\end{verbatim}

Командата за възпроизвеждане на Mode B анализа е:

\begin{verbatim}
cd Year-1/Semester-2/Recommendation-Systems/Assignment/nutri-evidence-agent
python3 scripts/analyze_openai_evaluation_results.py \
  --inputs \
    data/evaluation_annotations_openai_mesh_overlap_article_37656239.csv \
    data/evaluation_annotations_openai_semantic_article_37656239.csv \
    data/evaluation_annotations_openai_graph_article_37656239.csv \
    data/evaluation_annotations_openai_hybrid_article_37656239.csv \
  --output-dir docs/evaluation/article_37656239
\end{verbatim}

\vspace{0.5em}\hrule\vspace{1.0em}

\section{10. Ограничения}\label{ux43eux433ux440ux430ux43dux438ux447ux435ux43dux438ux44f}

Проектът има следните ограничения:

\begin{itemize}
\tightlist
\item
  работи основно с PubMed abstracts, не с full-text статии;
\item
  качеството зависи от наличието на abstract и MeSH terms;
\item
  semantic similarity не гарантира методологична релевантност;
\item
  node2vec embeddings зависят от размера и свързаността на графа;
\item
  локалният Llama 8B модел може да връща невалиден JSON или непълни explanations;
\item
  системата не оценява реално risk of bias;
\item
  OpenAI Judge оценките могат да съдържат грешки и трябва да се третират като assisted annotation, не като абсолютен ground truth;
\item
  системата не дава медицински съвети и не трябва да се използва за клинични решения.
\end{itemize}

\vspace{0.5em}\hrule\vspace{1.0em}

\section{11. Бъдеща работа}\label{ux431ux44aux434ux435ux449ux430-ux440ux430ux431ux43eux442ux430}

След MVP могат да се добавят:

\begin{itemize}
\tightlist
\item
  повече експерименти и таблици с резултати;
\item
  разширяване на OpenAI Judge evaluation с повече query-та и ръчна валидация;
\item
  FAISS или ChromaDB vector store;
\item
  Elasticsearch hybrid search;
\item
  MeSH hierarchy relations;
\item
  FoodOn ontology;
\item
  RDF triples и SPARQL заявки;
\item
  GraphSAGE или GCN;
\item
  user feedback loop;
\item
  export към Markdown, CSV или BibTeX;
\item
  full-text open-access retrieval;
\item
  risk-of-bias assessment.
\end{itemize}

\vspace{0.5em}\hrule\vspace{1.0em}

\section{12. Очакван принос на проекта}\label{ux43eux447ux430ux43aux432ux430ux43d-ux43fux440ux438ux43dux43eux441-ux43dux430-ux43fux440ux43eux435ux43aux442ux430}

Проектът демонстрира комбинация от няколко модерни AI и recommender system техники:

\begin{verbatim}
Biomedical information retrieval
Content-based recommender systems
Semantic similarity
Knowledge graphs
Graph embeddings
node2vec
Hybrid recommendation
Local LLM agents
RAG
LLM reranking
OpenAI Judge-assisted evaluation
Evaluation metrics
Streamlit demo
\end{verbatim}

Това го прави подходящ както за дисциплината „Препоръчващи системи``, така и като разширение към проект по „База от знания``.

\vspace{0.5em}\hrule\vspace{1.0em}

\section{13. Заключение}\label{ux437ux430ux43aux43bux44eux447ux435ux43dux438ux435}

NutriEvidence Agent представлява образователна система за препоръчване на биомедицинска литература. Основният подход е semantic similarity върху title + abstract на PubMed статии, допълнен с MeSH-базиран knowledge graph, node2vec embeddings и локален LLM слой чрез Ollama.

Проектът е структуриран така, че първо да се реализира работещ MVP, а след това да се разшири с експериментална оценка. В текущата версия е добавен OpenAI Judge компонент, който подпомага evaluation процеса чрез relevance labels за препоръчаните статии. В следваща версия документацията ще бъде допълнена с реални резултати, comparison tables, анализ на хипотезите и окончателно приемане или отхвърляне на всяка хипотеза.


\end{document}
